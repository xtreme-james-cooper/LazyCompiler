...
primrec subst\<^sub>x\<^sub>e\<^sub>s :: "nat list \<Rightarrow> expr \<Rightarrow> expr" where
  "subst\<^sub>x\<^sub>e\<^sub>s xs (Var y) = Var (case lookup xs y of Some x \<Rightarrow> x | None \<Rightarrow> y)"
...
| "subst\<^sub>x\<^sub>e\<^sub>s xs (Rec ys) = Rec (map (\<lambda>y. case lookup xs y of Some x \<Rightarrow> x | None \<Rightarrow> y) ys)"
| "subst\<^sub>x\<^sub>e\<^sub>s xs (Proj e l) = Proj (subst\<^sub>x\<^sub>e\<^sub>s xs e) l"
| "subst\<^sub>x\<^sub>e\<^sub>s xs (Inj l ts y) = Inj l ts (case lookup xs y of Some x \<Rightarrow> x | None \<Rightarrow> y)"
...
| "subst\<^sub>x\<^sub>e\<^sub>s xs (Fold t y) = Fold t (case lookup xs y of Some x \<Rightarrow> x | None \<Rightarrow> y)"
