    ...
    thus ?case 
        ...
        thus ?thesis
          ...
          next case False
            ...
            moreover with tc_app obtain t' where "(\<Delta>,\<Gamma> \<turnstile>\<^sub>s s\<^sub>2 : t' \<rightarrow> t\<^sub>1) \<and> \<Delta>,\<Gamma> \<turnstile> e' : t'" 
              by fastforce
            moreover with tc_app have "\<Delta>,\<Gamma> \<turnstile>\<^sub>s App e\<^sub>1 # s\<^sub>2 : t' \<rightarrow> t\<^sub>2" by fastforce
      ...
      next case False
        ...
        moreover with tc_app obtain t' where "(\<Delta>,\<Gamma> \<turnstile>\<^sub>s s\<^sub>1 : t' \<rightarrow> Arrow t\<^sub>1 t\<^sub>2) \<and> \<Delta>,\<Gamma> \<turnstile> e' : t'" 
          by fastforce
        moreover with tc_app have "\<Delta>,\<Gamma> \<turnstile>\<^sub>s (\<lambda>e. App e e\<^sub>2) # s\<^sub>1 : t' \<rightarrow> t\<^sub>2" by fastforce
        ultimately show ?thesis by fastforce
      qed
      ...
      next case False
        ...
        with tc_rec False obtain t t' vts nvts where T: "ts = vts @ [t] @ nvts \<and> 
          (\<Delta>,\<Gamma> \<turnstile>\<^sub>s ss : t' \<rightarrow> t) \<and> (\<Delta>,\<Gamma> \<turnstile> e' : t') \<and> (\<Delta>,\<Gamma> \<turnstile>\<^sub>f vs : vts) \<and> \<Delta>,\<Gamma> \<turnstile>\<^sub>f nvs : nvts" 
            by fastforce
        hence "\<Delta>,\<Gamma> \<turnstile>\<^sub>s (\<lambda>e. Rec (vs @ [e] @ nvs)) # ss : t' \<rightarrow> Record ts" by fastforce
    ...
    thus ?case
      ...
      next case False
        ...
        moreover with tc_proj obtain t' where "(\<Delta>,\<Gamma> \<turnstile>\<^sub>s ss : t' \<rightarrow> Record ts) \<and> (\<Delta>,\<Gamma> \<turnstile> e' : t')" 
          by fastforce
        moreover with tc_proj have "\<Delta>,\<Gamma> \<turnstile>\<^sub>s (\<lambda>e. Proj e l) # ss : t' \<rightarrow> t" by fastforce
        ultimately show ?thesis by fastforce
      qed
  next case (tc_inj \<Delta> \<Gamma> e t l ts)
    thus ?case
      ...
      next case False
        ...
        moreover with tc_inj S have "\<Delta>,\<Gamma> \<turnstile>\<^sub>s s : t' \<rightarrow> Variant ts" by fastforce
        ultimately show ?thesis by fastforce
      qed
    ...
    thus ?case
      ...
      next case False
        ...
        with tc_case obtain t' where T: "(\<Delta>,\<Gamma> \<turnstile>\<^sub>s s' : t' \<rightarrow> Variant ts) \<and> (\<Delta>,\<Gamma> \<turnstile> e' : t')" 
          by fastforce
        with tc_case S have "\<Delta>,\<Gamma> \<turnstile>\<^sub>s s : t' \<rightarrow> t" by fastforce
