theory Typecheck
imports Vector Expression Kindcheck
begin

inductive typecheck :: "nat \<Rightarrow> type list \<Rightarrow> expr \<Rightarrow> type \<Rightarrow> bool" (infix ",_ \<turnstile> _ :" 60) 
      and typecheck_fs :: "nat \<Rightarrow> type list \<Rightarrow> expr list \<Rightarrow> type list \<Rightarrow> bool" (infix ",_ \<turnstile>\<^sub>f _ :" 60)
      and typecheck_cs :: "nat \<Rightarrow> type list \<Rightarrow> expr list \<Rightarrow> type list \<Rightarrow> type \<Rightarrow> bool" 
...
lemma [simp]: "\<Delta>,\<Gamma> \<turnstile> e : t \<Longrightarrow> Suc \<Delta>,map (incr\<^sub>t\<^sub>t x) \<Gamma> \<turnstile> incr\<^sub>t\<^sub>e x e : incr\<^sub>t\<^sub>t x t"
  and [simp]: "\<Delta>,\<Gamma> \<turnstile>\<^sub>f fs : ts \<Longrightarrow> Suc \<Delta>,map (incr\<^sub>t\<^sub>t x) \<Gamma> \<turnstile>\<^sub>f map (incr\<^sub>t\<^sub>e x) fs : map (incr\<^sub>t\<^sub>t x) ts"
  and [simp]: "\<Delta>,\<Gamma> \<turnstile>\<^sub>c cs : ts \<rightarrow> t \<Longrightarrow>
    Suc \<Delta>,map (incr\<^sub>t\<^sub>t x) \<Gamma> \<turnstile>\<^sub>c map (incr\<^sub>t\<^sub>e x) cs : map (incr\<^sub>t\<^sub>t x) ts \<rightarrow> incr\<^sub>t\<^sub>t x t "
  ...
  next case (tc_inj \<Delta> \<Gamma> e t l ts)
    hence "Suc \<Delta>,map (incr\<^sub>t\<^sub>t x) \<Gamma> \<turnstile> incr\<^sub>t\<^sub>e x e : incr\<^sub>t\<^sub>t x t" by simp
    moreover from tc_inj have "lookup l (map (incr\<^sub>t\<^sub>t x) ts) = Some (incr\<^sub>t\<^sub>t x t)" by simp
    moreover from tc_inj have "\<forall>tt \<in> set (map (incr\<^sub>t\<^sub>t x) ts). Suc \<Delta> \<turnstile>\<^sub>k tt" by simp
    ultimately show ?case by simp
  next case (tc_case \<Delta> \<Gamma> e ts cs t)
    hence "Suc \<Delta>,map (incr\<^sub>t\<^sub>t x) \<Gamma> \<turnstile> incr\<^sub>t\<^sub>e x e : Variant (map (incr\<^sub>t\<^sub>t x) ts)" by simp
  ...
  next case (tc_tyapp \<Delta> \<Gamma> e t t')
    ...
    ultimately have "Suc \<Delta>,map (incr\<^sub>t\<^sub>t x) \<Gamma> \<turnstile> TyApp (incr\<^sub>t\<^sub>e x e) (incr\<^sub>t\<^sub>t x t') : 
      subst\<^sub>t\<^sub>t 0 (incr\<^sub>t\<^sub>t x t') (incr\<^sub>t\<^sub>t (Suc x) t)"
        by (metis typecheck_typecheck_fs_typecheck_cs.tc_tyapp)
    thus ?case by simp
  qed simp_all

lemma tc_subst\<^sub>t\<^sub>e [simp]: "Suc \<Delta>,\<Gamma> \<turnstile> e : t \<Longrightarrow> \<Delta> \<turnstile>\<^sub>k t' \<Longrightarrow> x \<le> \<Delta> \<Longrightarrow> 
    \<Delta>,map (subst\<^sub>t\<^sub>t x t') \<Gamma> \<turnstile> subst\<^sub>t\<^sub>e x t' e : subst\<^sub>t\<^sub>t x t' t"
  and [simp]: "Suc \<Delta>,\<Gamma> \<turnstile>\<^sub>f fs : ts \<Longrightarrow> \<Delta> \<turnstile>\<^sub>k t' \<Longrightarrow> x \<le> \<Delta> \<Longrightarrow> 
    \<Delta>,map (subst\<^sub>t\<^sub>t x t') \<Gamma> \<turnstile>\<^sub>f map (subst\<^sub>t\<^sub>e x t') fs : map (subst\<^sub>t\<^sub>t x t') ts"
  and [simp]: "Suc \<Delta>,\<Gamma> \<turnstile>\<^sub>c cs : ts \<rightarrow> t \<Longrightarrow> \<Delta> \<turnstile>\<^sub>k t' \<Longrightarrow> x \<le> \<Delta> \<Longrightarrow> 
  ...
  by (induction "Suc \<Delta>" \<Gamma> e t and "Suc \<Delta>" \<Gamma> fs ts and "Suc \<Delta>" \<Gamma> cs ts t 
...
lemma [simp]: "\<Delta>,\<Gamma> \<turnstile> e : t \<Longrightarrow> x \<le> length \<Gamma> \<Longrightarrow> \<Delta>,insert_at x t' \<Gamma> \<turnstile> incr\<^sub>e\<^sub>e x e : t"
  ...
  next case (tc_inj \<Delta> \<Gamma> e t l ts)
    ...
    moreover from tc_inj have "\<forall>tt \<in> set ts. \<Delta> \<turnstile>\<^sub>k tt" by simp
    ultimately show ?case by simp
  next case (tc_case \<Delta> \<Gamma> e ts cs t)
...
lemma [simp]: "\<Delta>,insert_at x t' \<Gamma> \<turnstile> e : t \<Longrightarrow> x \<le> length \<Gamma> \<Longrightarrow> \<Delta>,\<Gamma> \<turnstile> e' : t' \<Longrightarrow> \<Delta>,
    \<Gamma> \<turnstile> subst\<^sub>e\<^sub>e x e' e : t"
